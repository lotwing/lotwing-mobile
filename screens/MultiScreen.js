...
class LotView extends React.Component {
  constructor(props) {
   ...
   componentWillMount() {
    this.props.navigation.setParams({ section: 'multi', onPress: () => { this.setState({ newVehicleSpaces: [], usedVehicleSpaces: []}) }})
   }
  ...
  setModalVisibility(visibility, modalType = null, vehicleId = null, opt_basic_modal_action_fb_msg = null) {
    ...
    if (modalType !== null) {
      let textToShow = null
      if (modalType == GlobalVariables.ACTION_FEEDBACK_MODAL_TYPE && visibility == false && opt_basic_modal_action_fb_msg) {
  ...
  setLocalHighlight(spaceId, vehicle) {
    ...
    if (this.state.key_board !== null) {
      ...
      .then((response) => {
          ...
          this.setState({ usedVehicleSpaces: usedSpaces, newVehicleSpaces: newSpaces,  modalType: GlobalVariables.EMPTY_MODAL_TYPE, modalVisible: false })
      })
      .catch(err => {
  ...
  maybeRenderKeyboard() {
    if (this.state.key_board_locations.length > 0) {
      return(
        <View style={{ position: 'absolute', zIndex: 10, width: Dimensions.get('window').width - 40, left: 20, top: 40, backgroundColor: '#828282', borderRadius: 10 }}>
          <TouchableOpacity onPress={()=> this.setState({ key_board_open: !this.state.key_board_open })}>
            <View style={{ padding: 10, paddingLeft: 14, paddingRight: 14 }}>
              <Text style={{ color: '#FFF'}}>{ this.state.key_board !== null ? `KEY BOARD LOCATION: ${ this.state.key_board}` : 'SELECT A KEY BOARD'}</Text>
            </View>
          </TouchableOpacity>
          { this.state.key_board_open &&
            <View>
              { this.state.key_board_locations.map((keylocation) => {
                return(
                  <TouchableOpacity key={ keylocation.id} onPress={()=> this.setState({ key_board: keylocation.name, key_board_open: false })}>
                    <View style={[{ padding: 10, paddingLeft: 14, paddingRight: 14 }, keylocation.name === this.state.key_board && { backgroundColor: '#727272'} ]}>
