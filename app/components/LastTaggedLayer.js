 ...
 */
export default class LastTaggedLayer extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  getAllEventsCoordinatesObject() {
    console.log('EVENT SHAPES: ', this.props.eventShapes);
  ...
  renderEvents() {
    ...
    if (event_coord_obj) {
      let polygons = Object.keys(event_coord_obj).map(ps_id =>
        this._createNewPolygon(event_coord_obj.geometry.coordinates, ps_id),
      );
