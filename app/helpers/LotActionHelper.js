...
export default {
  ...
  endTimeboundTagAction: function(actionPayload, eventId) {
    ...
    return fetch(
      GlobalVariables.BASE_ROUTE + Route.COMPLETE_TIMED_TAG_EVENT + eventId,
      {
        method: 'PUT',
        headers: {
          Accept: 'application/json',
  ...
  getEventId: function(spaceId, eventType) {
    ...
    return fetch(
      GlobalVariables.BASE_ROUTE + Route.VEHICLE_BY_SPACE + spaceId,
      {
        method: 'GET',
        headers: {
          Accept: 'application/json',
      ...
      .then(response => {
        ...
        const targetEvents = response.events[0].filter(
          event => event.data.attributes.event_type === eventType,
        );
  ...
  createPolygonsFromObject: function(polygonObject) {
    return Object.keys(polygonObject).map(ps_id =>
      this._createNewPolygon(
        polygonObject[ps_id].geo_info.geometry.coordinates,
        ps_id,
      ),
    );
  },
